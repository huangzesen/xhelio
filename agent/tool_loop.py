...
def run_tool_loop(
    ...
    Args:
        ...
        terminal_tools: Optional set of tool names that signal "done".
            When a tool in this set is called, the loop breaks after
            processing all tool calls in the current batch.
    ...
    while True:
        ...
        for fc in function_calls:
            ...
            # Duplicate call detection — skip execution if hard-blocked
            ...
            if verdict.blocked:
                ...
                get_event_bus().emit(
                    DEBUG, agent=agent_name, level="warning",
                    msg=f"[{agent_name}] {tool_name} BLOCKED (dup #{verdict.count})",
                )
            ...
            if isinstance(result, dict):
                ...
                # Inject duplicate warning if applicable
                if verdict.warning:
            ...
            # Check if this tool signals loop completion
            if terminal_tools and tool_name in terminal_tools:
                hit_terminal = True

            if result.get("status") == "error":
                get_event_bus().emit(TOOL_ERROR_LOG, agent=agent_name, level="warning", msg=f"[{agent_name}] Tool error: {result.get('message', '')}")
                # Track invalid/hallucinated tool names for early loop termination
                err_msg = result.get("message", "")
                if "not available for" in err_msg or "Unknown tool" in err_msg:
                    guard.record_invalid_tool(tool_name)
        ...
        # Check for repeated invalid/hallucinated tool calls
        invalid_stop = guard.check_invalid_tool_limit()
        if invalid_stop:
            get_event_bus().emit(DEBUG, agent=agent_name, level="warning", msg=f"[{agent_name}] Tool loop stopping: {invalid_stop}")
            break

        # Terminal tool was called — stop looping
        if hit_terminal:
            get_event_bus().emit(DEBUG, agent=agent_name, level="debug", msg=f"[{agent_name}] Tool loop stopping: terminal tool called")
            break
