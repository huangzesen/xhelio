...
def log_error(
    ...
    """
    from .event_bus import get_event_bus, TOOL_ERROR
    ...
    # Emit as error event — DebugLogListener writes to file+console,
    # SSEEventListener forwards to frontend
    bus = get_event_bus()
    bus.emit(TOOL_ERROR, level="error", msg=full_message,
             data={"short": message[:200], "context": context or {}})
...
_MILESTONE_TAGS = frozenset({
    ...
    "progress",         # Milestone updates during planning/discovery
    "data_fetched",     # "[DataOps] Stored 'label' (N points)"
    "thinking",         # "[Thinking] ..." (collapsed with other milestones)
})
