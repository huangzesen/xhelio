...
def handle_pipeline(orch: "OrchestratorAgent", tool_args: dict) -> dict:
    action = tool_args.get("action", "")

    if action == "info":
        return _handle_pipeline_info(orch, tool_args)
    elif action == "modify":
        return _handle_pipeline_modify(orch, tool_args)
    elif action == "execute":
        return _handle_pipeline_execute(orch, tool_args)
    elif action == "save":
        return _handle_pipeline_save(orch, tool_args)
    elif action == "run":
        return _handle_pipeline_run(orch, tool_args)
    elif action == "search":
        return _handle_pipeline_search(orch, tool_args)
    else:
        return {"status": "error", "message": f"Unknown action: {action}"}


def _handle_pipeline_info(orch: "OrchestratorAgent", tool_args: dict) -> dict:
...
def _handle_pipeline_modify(orch: "OrchestratorAgent", tool_args: dict) -> dict:
    sub_action = tool_args.get("sub_action", "")

    modified_args = dict(tool_args)
    if sub_action:
        modified_args["action"] = sub_action

    return handle_modify_pipeline_node(orch, modified_args)


def _handle_pipeline_execute(orch: "OrchestratorAgent", tool_args: dict) -> dict:
    return handle_execute_pipeline(orch, tool_args)


def _handle_pipeline_save(orch: "OrchestratorAgent", tool_args: dict) -> dict:
    return handle_save_pipeline(orch, tool_args)


def _handle_pipeline_run(orch: "OrchestratorAgent", tool_args: dict) -> dict:
    return handle_run_pipeline(orch, tool_args)


def _handle_pipeline_search(orch: "OrchestratorAgent", tool_args: dict) -> dict:
    return handle_search_pipelines(orch, tool_args)
...
def handle_run_pipeline(orch: "OrchestratorAgent", tool_args: dict) -> dict:
    ...
    for (delegation_type, mission_id), calls in batches:
        ...
        if delegation_type == "mission":
            ...
            agent = orch._get_or_create_envoy_agent(mission_id)
            sub_result = agent.send(
                instruction, sender="pipeline_replay", timeout=300.0, wait=True
            )
            orch._event_bus.emit(
                DELEGATION_DONE,
                level="debug",
            ...
            _eph = agent.agent_id != "DataOpsAgent"
            sub_result = agent.send(
                instruction, sender="pipeline_replay", timeout=300.0, wait=True
            )
            if _eph:
                orch._cleanup_ephemeral_agent(agent.agent_id)
            orch._event_bus.emit(
                DELEGATION_DONE,
                level="debug",
