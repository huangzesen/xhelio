...
EUREKA_SUGGESTION = "eureka_suggestion"
EUREKA_CHAT_MESSAGE = "eureka_chat_message"
EUREKA_CHAT_RESPONSE = "eureka_chat_response"
...
INFRASTRUCTURE_TAGS: dict[str, frozenset[str]] = {
    ...
    EUREKA_SUGGESTION: frozenset({"display", "console"}),
    EUREKA_CHAT_MESSAGE: frozenset({"display", "console"}),
    EUREKA_CHAT_RESPONSE: frozenset({"display", "console"}),
    # Context compaction (message-level)
    CONTEXT_COMPACTION: frozenset({"display", "console"}),
    # Persistent event loop (turnless orchestrator)
...
def _tool_linked_ctx_tags(event_type: str) -> frozenset[str]:
    ...
    if not linked:
        return frozenset()
    return frozenset(
        ctx for ctx, tools in AGENT_INFORMED_REGISTRY.items() if linked & tools
    )
...
class EventBus:
    ...
    def emit(
        ...
        *,
        ...
        if not summary and msg:
            ...
            summary, details = format_event(
                type, agent, effective_data, list(children) if children else None
            )
    ...
    def get_events_since(self, index: int) -> list[SessionEvent]:
        """Return events from index onward."""
        with self._lock:
                ...
                if event.type == THINKING:
                    ...
                    if tool_result is not None:
                        if isinstance(tool_result, dict):
                            result_text = json.dumps(tool_result, ensure_ascii=False, indent=1)
                        else:
                            result_text = str(tool_result)
                        # Truncate for display (removed limit - frontend handles scrolling)
                    self._callback(
                        {
                            "type": TOOL_RESULT,
                            "tool_name": event.data.get("tool_name", ""),
                    ...
                    self._callback(
                        {
                            ...
                            "round_token_usage": event.data.get(
                                "round_token_usage", {}
                            ),
                        }
                    )
