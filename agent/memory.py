...
def _git_commit_data(repo_dir: Path) -> None:
    ...
    try:
        ...
        # Stage and commit — only include pipelines/ if the directory exists
        add_paths = ["memory.json", ".gitignore"]
        if (repo_dir / "pipelines").is_dir():
            add_paths.append("pipelines/")
        subprocess.run(
            ["git", "add"] + add_paths,
            **run_kwargs,
            check=True,
...
def generate_tags(content: str, scopes: list[str] | str = "generic") -> list[str]:
    ...
    tags = list(
        dict.fromkeys(t for t in tokens if t and len(t) >= 2 and t not in _STOP_WORDS)
    )
...
class MemoryEmbeddings:
    ...
    def _ensure_model(self) -> bool:
        ...
        try:
            ...
            MemoryEmbeddings._shared_model = TextEmbedding(
            ...
            get_event_bus().emit(
                ...
                agent="Memory",
                level="debug",
                msg="[MemoryEmbeddings] fastembed unavailable, falling back to tag-based methods",
            )
        return MemoryEmbeddings._shared_available
    ...
    def build(self, memories) -> None:
        ...
        self._embeddings = np.array(
            list(MemoryEmbeddings._shared_model.embed(self._contents))
        )
...
class Memory:
    ...
    source: str = (
        "extracted"  # "extracted" | "reflected" | "user_explicit" | "consolidated"
    )
    ...
    review_of: str = (
        ""  # non-empty only for type="review" — ID of the memory being reviewed
    )
...
class MemoryStore(VersionedStore[Memory]):
        ...
        for raw in raw_memories:
            try:
                ...
                get_event_bus().emit(
                    ...
                    agent="Memory",
                    level="warning",
                    msg=f"[Memory] Skipping malformed entry: {e}",
                )
        return result
    ...
    def execute_actions(self, actions: list[dict], session_id: str = "") -> int:
        ...
        def _validate_scopes(scopes_raw) -> list[str]:
            ...
            if not isinstance(scopes_raw, list):
                return ["generic"]
            valid = [
                s for s in scopes_raw if isinstance(s, str) and _valid_scope_re.match(s)
            ]
        ...
        for action in actions:
            ...
            try:
                ...
                get_event_bus().emit(
                    ...
                    agent="Memory",
                    level="debug",
                    msg=f"[Memory] execute_actions: {action_type} failed — {e}",
                )
    ...
    def total_tokens(self) -> int:
        ...
        return sum(
            estimate_memory_tokens(m)
            for m in self._entries
            if m.enabled and not m.archived
        )
    ...
    def get_recent_reviews_for_lineage(
        self, memory_id: str, n: int = 10
    ) -> list[Memory]:
    ...
    def _search_by_embeddings(
        ...
        try:
            ...
            scored = [
                ...
                if sims[i]
                > 0.55  # minimum similarity threshold (bge-small has ~0.5 baseline)
            ]
    ...
    def _agent_name_for_scope(scope: str) -> str:
        ...
        if scope == "visualization":
            ...
            return "DataOpsAgent"
        elif scope.startswith("envoy:"):
            return f"EnvoyAgent[{scope.split(':', 1)[1]}]"
    ...
    def format_for_injection(
        ...
        Args:
            ...
            active_missions: Set of active mission IDs (e.g. ``{"ACE", "WIND"}``).
                When provided, memories that have a ``envoy:<ID>`` scope are
                only included if that mission is in this set.  Memories without
                any envoy scope are unaffected.  Useful for filtering
                irrelevant envoy-specific memories from the DataOps agent.
        ...
        if active_missions is not None:
            filtered = []
            for m in matching:
                mission_scopes = [s for s in m.scopes if s.startswith("envoy:")]
                if mission_scopes:
                    # Memory is envoy-specific — only include if at least one
                    # of its envoy scopes is currently active
                    if any(
                        s.split(":", 1)[1] in active_missions for s in mission_scopes
                    ):
                        filtered.append(m)
                else:
                    # No envoy scope — always include
                    filtered.append(m)
            matching = filtered
        if not matching:
            return ""
        ...
        summaries = (
            [m for m in matching if m.type == "summary"] if include_summaries else []
        )
        ...
        if include_review_instruction:
            parts.append("")
            parts.append(
                "IMPORTANT: After completing your main task, you MUST call "
                "review_memory(memory_id, stars, rating, criticism, suggestion, comment) "
