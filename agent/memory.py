...
MEMORY_TOKEN_BUDGET = config.get("memory_token_budget", 10000)


def _git_commit_memory(memory_path: Path) -> None:
    """Auto-commit memory.json to git for full change history.

    Lazily initializes a git repo in memory_path.parent (~/.xhelio/) if needed.
    Never raises — all errors are logged and swallowed.
    """
    repo_dir = memory_path.parent
    ...
    try:
        ...
        if not (repo_dir / ".git").exists():
            ...
            # .gitignore: track only memory.json
            gitignore = repo_dir / ".gitignore"
            gitignore.write_text("# Track only memory.json\n*\n!.gitignore\n!memory.json\n")

        # Stage and commit
        subprocess.run(
            ["git", "add", "memory.json", ".gitignore"],
            **run_kwargs, check=True,
        )
        ...
        if result.returncode != 0:
            ...
            subprocess.run(
                ["git", "commit", "-m", f"memory: auto-save {timestamp}"],
                **run_kwargs, check=True,
            )
...
def estimate_memory_tokens(memory: "Memory") -> int:
    """Estimate tokens for a single memory rendered as '- {content}'."""
    return estimate_tokens(f"- {memory.content}")
...
class MemoryEmbeddings:
    ...
    def _ensure_model(self) -> bool:
        """Lazy-init the embedding model. Returns True if available."""
    ...
    def build(self, memories) -> None:
        ...
        if not self._contents:
            ...
            return
        self._embeddings = np.array(list(self._model.embed(self._contents)))

    def embed_query(self, text: str) -> np.ndarray | None:
        """Embed a single query string. Returns None if unavailable."""
        if not self._ensure_model():
            return None
        return np.array(list(self._model.embed([text])))[0]
    ...
    def pairwise_max_similarity(self, text: str, others: list[str]) -> float:
        ...
        try:
            ...
            embs = np.array(list(self._model.embed(all_texts)))
            query_emb = embs[0]
            other_embs = embs[1:]
...
class MemoryStore(VersionedStore[Memory]):
    ...
    def _post_load_fixup(self) -> bool:
        """Generate tags for memories that have none + import cold storage."""
        needs_save = False
        ...
        # Import cold storage on first v4 load
        needs_save |= self._import_cold_storage()

        return needs_save

    def _import_cold_storage(self) -> bool:
        """Import memory_cold.json entries as archived=True into main file.

        Only runs once — checks for cold file existence.
        Returns True if entries were imported.
        """
        ...
        # Check if we already imported (don't re-import)
        existing_ids = {m.id for m in self._entries}
        imported = 0
        for raw in cold_entries:
            # Normalize old cold entries
            if "scope" in raw and "scopes" not in raw:
                raw["scopes"] = [raw.pop("scope")]
            raw.pop("scope", None)
            raw.pop("confidence", None)
            raw.setdefault("source", "extracted")
            raw.setdefault("tags", [])
            raw.setdefault("supersedes", "")
            raw.setdefault("access_count", 0)
            raw.setdefault("last_accessed", "")
            raw.setdefault("version", 1)
            raw["archived"] = True  # Always archived
        ...
        if imported > 0:
            get_event_bus().emit(DEBUG, agent="Memory", level="debug", msg=f"[Memory] Imported {imported} entries from cold storage")
            return True
        return False
    ...
    def archive_to_cold(self, memories: list[Memory]) -> None:
        """Deprecated: archives memories in-file instead.

        For backward compatibility, marks the given memories as archived.
        """
        if not memories:
            return
        ...
        for m in self._entries:
            if m.id in memory_ids:
                m.archived = True
        self.save()
        get_event_bus().emit(DEBUG, agent="Memory", level="debug", msg=f"[Memory] Archived {len(memories)} memories in-file (cold storage deprecated)")

    def read_cold(self) -> list[dict]:
        """Load archived memories as dicts (replaces cold file reads).

        Returns archived entries from the main file.
        """
        return [asdict(m) for m in self._entries if m.archived]
    ...
    def search_cold(self, query: str, mem_type: str | None = None, limit: int = 20) -> list[dict]:
        """Search truly archived memories by substring match on content.

        Only searches memories that were dropped/deleted, not versioned
        predecessors of still-living memories.
        """
        entries = [asdict(m) for m in self.get_truly_archived()]
    ...
    def format_for_injection(
        ...
        _add_items(
            preferences,
            ["", "### Preferences"],
            lambda m: f"- [{m.id}] {m.content}",
        )
        _add_items(
            summaries,
            ["", "### Past Sessions"],
            lambda m: f"- [{m.id}] ({m.created_at[:10]}) {m.content}" if m.created_at else f"- [{m.id}] {m.content}",
        )
        _add_items(
            pitfalls,
            ["", "### Lessons Learned"],
        ...
        if include_review_instruction:
            ...
            parts.append(
                ...
                "for at least 1 (up to 4) of the memories listed above. This is required."
            )
