...
class ContextTracker:
    ...
    def get_store_delta(
        ...
        Returns:
            (new_entries, removed_ids, full_hash)
        """
        ...
        current_ids = frozenset(e.get("id", "") for e in entries if e.get("id"))

        if snap:
            new_entries = [e for e in entries if e.get("id", "") not in snap.store_ids]
            removed_ids = sorted(snap.store_ids - current_ids)
            # If no new/removed but hash differs, metadata changed (e.g. point count)
            # — send the changed entries
            if not new_entries and not removed_ids:
                new_entries = entries  # re-send all (metadata update)
            return new_entries, removed_ids, full_hash
    ...
    def record(self, agent_id: str, **kwargs) -> None:
        ...
        if "store_entries" in kwargs:
            snap.store_ids = frozenset(
                e.get("id", "") for e in kwargs["store_entries"] if e.get("id")
            )
