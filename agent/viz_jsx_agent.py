"""
JSX/Recharts visualization agent with optional think phase.
...
For component-creation requests, runs a think→execute pattern: inspect data
shapes, types, units, and NaN counts before generating JSX.
...
from .agent_registry import VIZ_JSX_TOOLS, VIZ_THINK_TOOLS
...
class VizJsxAgent(SubAgent):
    """A SubAgent specialized for JSX/Recharts visualization with think→execute pattern.
    ...
    def __init__(
        ...
        *,
        ...
        # Think-phase tool schemas (data inspection only — shared with Plotly/MPL agents)
    ...
    def _handle_request(self, msg):
        """Override to inject think→execute pattern for component-creation requests."""
        ...
        if _needs_think_phase(content):
            ...
            if think_context:
                content = (
                    ...
                    f"Now create the visualization using generate_jsx_component.\n"
                    f"Use the data labels and column info from the findings above."
                )
        else:
            self._event_bus.emit(
                DEBUG, agent=self.agent_id,
                msg="[JsxViz] Skipping think phase (style/manage request)",
            )
    ...
    def _run_think_phase(self, user_request: str) -> str:
        ...
        self._event_bus.emit(
            DEBUG, agent=self.agent_id,
            msg="[JsxViz] Think phase: inspecting data...",
        )
        ...
        self._event_bus.emit(
            ...
            msg="[JsxViz] Think phase complete",
        )
        return text or ""
