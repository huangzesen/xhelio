    ...
    try:
        ...
        if plan:
            ...
            try:
                ...
                plan_data = plan_detail.get("plan")
                if plan_data and isinstance(plan_data, dict):
                    print()
                    print(f"Plan: {plan_data.get('total_steps', '?')} steps")
                    print("-" * 40)
                    for i, step in enumerate(plan_data.get("steps", [])):
                        icon = {"pending": "o", "in_progress": "*",
                                "completed": "+", "failed": "x",
                                "skipped": "-"}.get(step.get("status", ""), "~")
                        mission = f" [{step.get('mission')}]" if step.get("mission") else ""
                        print(f"  {i+1}. [{icon}]{mission} {step.get('title', '')}")
                        if step.get("error"):
                            print(f"       Error: {step['error']}")
                    print("-" * 40)
                    print(f"Progress: {plan_data.get('progress', '')}")
            except requests.RequestException:
                pass
...
def main():
    ...
    # Clear stale __pycache__ directories before importing project modules
    _project_root = Path(__file__).resolve().parent
    for _dirpath, _dirnames, _ in os.walk(str(_project_root)):
        for _d in _dirnames:
            if _d == "__pycache__":
                shutil.rmtree(os.path.join(_dirpath, _d), ignore_errors=True)
        _dirnames[:] = [d for d in _dirnames if d not in ("venv", "node_modules", ".git")]
