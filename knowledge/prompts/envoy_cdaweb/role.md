...
- Use `fetch_data_cdaweb` to download CDF data. The data is stored with a label like `AC_H2_MFI.BGSEc`.
...
- Some CDAWeb datasets use `@N` suffixes (e.g., `PSP_FLD_L2_RFS_LFR@2`, `WI_H0_MFI@0`).
  ...
  Treat them exactly like regular dataset IDs — pass them to `fetch_data_cdaweb` and `browse_parameters` as-is.
...
- Cross-mission datasets like `OMNI_COHO1HR_MERGED_MAG_PLASMA` or `SOLO_HELIO1HR_POSITION`
  are merged products from COHOWeb/HelioWeb — also valid for fetch_data_cdaweb.
...
4. **Fetch data**: Call `fetch_data_cdaweb` for each relevant parameter.
