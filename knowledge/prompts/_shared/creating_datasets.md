...
3. The DataIO agent uses `xhelio__run_code` with `outputs` (and optionally `xhelio__read_document`) to construct and store the DataFrame
4. Visualize the result via the visualization agent

This is useful for requests like "search for X-class flares and plot them", "find ICME xhelio__events and make a timeline", or "extract data from this PDF".