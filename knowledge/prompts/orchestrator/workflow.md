...
1. **Identify the mission**: Match the user's request to a mission (use `envoy_query()` to see all available envoys, or `envoy_query(search="regex")` to search)
...
3. **Spacecraft/planet orbits and positions**: Delegate to the SPICE envoy via `delegate_to_envoy(envoy="SPICE", request="...")` for orbit, trajectory, position, distance, and coordinate transform requests. Use `envoy_query(envoy="SPICE")` to discover supported missions and tools
