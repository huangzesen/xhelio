...
def _save_to_local_cache(dataset_id: str, info: dict) -> None:
    ...
    Preserves existing startDate/stopDate if the new info has empty values,
    since Master CDF skeletons don't contain date ranges but a prior
    rebuild/refresh may have populated them.
    """
    cache_filename = _dataset_id_to_cache_filename(dataset_id)

    def _preserve_dates(cache_file: Path, new_info: dict) -> dict:
        """Preserve startDate/stopDate from existing cache when new info lacks them."""
        if new_info.get("startDate") and new_info.get("stopDate"):
            return new_info  # New info already has dates
        if not cache_file.exists():
            return new_info
        try:
            old = json.loads(cache_file.read_text(encoding="utf-8"))
            if not new_info.get("startDate") and old.get("startDate"):
                new_info["startDate"] = old["startDate"]
            if not new_info.get("stopDate") and old.get("stopDate"):
                new_info["stopDate"] = old["stopDate"]
        except (json.JSONDecodeError, OSError):
            pass
        return new_info
    ...
    if dataset_id.startswith("urn:nasa:pds:") or dataset_id.startswith("pds3:"):
        ...
        if mission_stem:
            ...
            try:
                _preserve_dates(cache_file, info)
...
def get_dataset_time_range(dataset_id: str) -> Optional[dict]:
    ...
    Checks two sources:
    1. Per-dataset metadata (startDate/stopDate from get_dataset_info)
    2. _index.json (start_date/stop_date from CDAWeb catalog API)

    Many per-dataset metadata files (from Master CDF skeletons) have empty
    startDate/stopDate. The _index.json is populated from the CDAWeb catalog
    API during generation and always has dates.
    ...
    # Fallback: look up _index.json when per-dataset metadata has empty dates
    if not start or not stop:
        local_path = _find_local_cache(dataset_id)
        if local_path is not None:
            # Path: .../missions/{source}/{mission_stem}/metadata/{ds}.json
            mission_stem = local_path.parent.parent.name
            index = list_cached_datasets(mission_stem)
            if index:
                for ds in index.get("datasets", []):
                    if ds.get("id") == dataset_id:
                        start = start or ds.get("start_date", "")
                        stop = stop or ds.get("stop_date", "")
                        break
...
def get_dataset_docs(dataset_id: str, max_chars: int | None = None) -> dict:
    ...
    documentation text scraped from the CDAWeb Notes page. Also returns
    full parameter list with metadata, time range, and validation info.
    ...
    Returns:
        Dict with dataset_id, contact, resource_url, documentation, parameters,
        time_range, and validation fields. documentation may be None if the
        Notes page could not be fetched or the dataset section was not found.
    """
    ...
    # Try to get full metadata from local cache or CDAWeb
    try:
        info = get_dataset_info(dataset_id)
        result["contact"] = info.get("contact")
        result["resource_url"] = info.get("resourceURL")

        # Add parameters with full metadata
        ...
        if params:
            result["parameters"] = [
                ...
                for p in params
                if p.get("name")  # Skip params without name
            ]
        ...
        stop = info.get("stopDate", "")
        if start or stop:
            result["time_range"] = {"start": start, "stop": stop}

        # Add validation status
        result["validated"] = info.get("_validated", False)
        validations = info.get("_validations", [])
        if validations:
            result["validation"] = {
                "validated": info.get("_validated", False),
                "validation_count": len(validations),
                "last_validated": validations[-1].get("validated_at"),
                "files_validated": [v.get("source_file") for v in validations if v.get("source_file")],
            }
    except Exception:
        pass
