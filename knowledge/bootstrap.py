...
def _generate_index(mission_stem: str):
    ...
    Reads parameter counts from per-dataset cache files. For dates, uses the
    mission JSON (instruments.*.datasets.*.start_date/stop_date) as the
    primary source — per-dataset cache files often have empty startDate/stopDate
    because Master CDF skeletons don't contain date ranges.
    """
    cache_dir = MISSIONS_DIR / mission_stem / "metadata"
    if not cache_dir.exists():
        return

    # Load mission JSON to get authoritative dates
    mission_json_path = MISSIONS_DIR / f"{mission_stem}.json"
    mission_id = mission_stem.upper()
    mission_dates: dict[str, dict] = {}  # ds_id -> {start_date, stop_date}
    ...
    for cache_file in sorted(cache_dir.glob("*.json")):
        ...
        # Dates: prefer per-dataset cache, fall back to mission JSON
        start_date = info.get("startDate", "")
        stop_date = info.get("stopDate", "")
        if not start_date or not stop_date:
            mj_dates = mission_dates.get(ds_id, {})
            start_date = start_date or mj_dates.get("start_date", "")
            stop_date = stop_date or mj_dates.get("stop_date", "")
