...
def _load_override(cache_key: str) -> dict | None:
    ...
    try:
        ...
        if not isinstance(data, dict):
            get_event_bus().emit(DEBUG, agent="MissionLoader", level="warning", msg=f"Override file {path} is not a JSON object; ignoring")
            return None
        return data
    except json.JSONDecodeError as exc:
        get_event_bus().emit(DEBUG, agent="MissionLoader", level="warning", msg=f"Malformed JSON in override file {path}: {exc}")
        return None
    except OSError as exc:
        get_event_bus().emit(DEBUG, agent="MissionLoader", level="warning", msg=f"Cannot read override file {path}: {exc}")
        return None
...
def _save_override(cache_key: str, data: dict) -> None:
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("\n")
    get_event_bus().emit(DEBUG, agent="MissionLoader", msg=f"Saved mission override: {path}")
