...
def _emit_debug(
    *, agent: str = "MissionLoader", level: str = "debug", msg: str = ""
) -> None:
...
def _load_override(cache_key: str) -> dict | None:
    ...
    try:
        ...
        if not isinstance(data, dict):
            _emit_debug(
                level="warning",
                msg=f"Override file {path} is not a JSON object; ignoring",
            )
            return None
        return data
    except json.JSONDecodeError as exc:
        _emit_debug(
            level="warning", msg=f"Malformed JSON in override file {path}: {exc}"
        )
        return None
...
def load_mission(mission_id: str) -> dict:
    ...
    *mission_id* (case-insensitive).  Each source directory is independent
    ...
    if cache_key not in _mission_cache:
        ...
        stem = (
            cache_key.removesuffix("_ppi") if cache_key.endswith("_ppi") else cache_key
        )
