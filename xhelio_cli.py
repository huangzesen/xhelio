...
Usage:
    ...
    xhelio purge        Reset sessions/memory (cached downloads preserved)
...
def _clear_pycache():
    """Remove all __pycache__ directories to avoid stale .pyc issues."""
    count = 0
...
def _print_paths():
    """Print data and cache directory paths at startup."""
    ...
    for dirpath, dirnames, _ in os.walk(str(PROJECT_ROOT)):
        for d in dirnames:
            if d == "__pycache__":
                shutil.rmtree(os.path.join(dirpath, d), ignore_errors=True)
                pycache_count += 1
        # Don't descend into venv/node_modules
        dirnames[:] = [d for d in dirnames if d not in ("venv", "node_modules", ".git")]
    if pycache_count:
...
def main():
    ...
    # xhelio purge
    subparsers.add_parser("purge", help="Reset sessions/memory (cached downloads preserved)")
