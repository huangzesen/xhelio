...
def _ensure_env():
    """Check .env exists; prompt for API key if missing."""
    env_path = PROJECT_ROOT / ".env"
    if env_path.exists():
        return
    print()
    print("No .env file found. Creating one — you need at least one API key.")
    print("See CLAUDE.md for details on supported providers.")
    print()
    try:
        api_key = input("Enter your API key (LLM_API_KEY): ").strip()
...
def main():
    parser = argparse.ArgumentParser(
        prog="xhelio",
        description="XHelio — AI-powered spacecraft data visualization",
    )
    parser.add_argument(
