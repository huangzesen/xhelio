...
export function ChatPage({ sidebarOpen, activityOpen }: Props) {
  ...
  const { messages, toolEvents, logLines, memoryEvents, isStreaming, clearChat } = useChatStore();
  ...
  const handleNewChat = useCallback(async () => {
    try {
      if (activeSessionId) {
        await api.deleteSession(activeSessionId).catch(() => {});
      }
      clearChat();
      await createSession();
    } catch {
            ...
            useChatStore.setState((s) => ({
              messages: [
                ...s.messages,
                {
                  ...
                  thumbnailSessionId: sess.activeSessionId,
                },
              ],
            }));
          }
        } catch {
          // No thumbnail — try fetching the full figure as fallback
          try {
            const { figure } = await api.getFigure(sess.activeSessionId);
            if (figure) {
              useChatStore.setState((s) => ({
                figureJson: figure,
                messages: [
