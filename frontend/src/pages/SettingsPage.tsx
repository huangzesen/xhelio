...
import { Save, Loader2, Check, RefreshCw, Database, Globe, Eye, EyeOff, CheckCircle2, XCircle, HardDrive, ChevronRight, ChevronDown, Activity } from 'lucide-react';
import { catalogStream } from '../api/sse';
import { listModels, getApiKeyStatus, updateApiKey, getMcpStatus, updateMcpConfig, getMcpActivity, type ModelInfo, type ApiKeyStatus, type McpStatus, type McpActivity } from '../api/client';
...
export function SettingsPage() {
  ...
  // Fetch MCP status when provider is minimax
  useEffect(() => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-panel rounded-xl border border-border p-4 space-y-4">
            ...
            {activeProvider === 'minimax' && (() => {
              const KNOWN_URLS = ['https://api.minimaxi.com/anthropic', 'https://api.minimax.io/anthropic'];
              const currentUrl = getProviderField('base_url') || KNOWN_URLS[0];
              const isCustomUrl = !KNOWN_URLS.includes(currentUrl);
              const showCustomInput = minimaxCustomMode || isCustomUrl;

              return (
            <label className="block">
              <span className="text-xs text-text-muted">Base URL</span>
              <select
                value={showCustomInput ? '__custom__' : currentUrl}
                onChange={(e) => {
              ...
              >
                <option value="https://api.minimaxi.com/anthropic">
                  China (api.minimaxi.com) — for users in China
                </option>
                <option value="https://api.minimax.io/anthropic">
                  International (api.minimax.io) — for users outside China
                </option>
              ...
              {showCustomInput && (
                <input
                  type="text"
                  value={isCustomUrl ? currentUrl : ''}
                  onChange={(e) => setProviderField('base_url', e.target.value)}
                  placeholder="https://your-custom-endpoint.com/anthropic"
                  className="mt-2 block w-full rounded-lg border border-border px-3 py-2 text-sm font-mono bg-input-bg text-text"
                />
              )}
            </label>
              );
            ...
            <label className="block">
              <span className="text-xs text-text-muted">
                API Rate Limit ({getProviderField('rate_limit_interval') || '2.0'}s between calls)
              </span>
              <input
                type="range"
                min={0.5}
                ...
                onChange={(e) => setProviderField('rate_limit_interval', parseFloat(e.target.value))}
                className="mt-1 block w-full"
              />
              {descriptions['providers.minimax.rate_limit_interval'] && (
                <p className="mt-1.5 text-xs italic text-text-muted/70 leading-relaxed">
                  {descriptions['providers.minimax.rate_limit_interval']}
                </p>
              )}
            </label>
            ...
            {/* MiniMax MCP Settings */}
            {activeProvider === 'minimax' && (
              <div className="border-t border-border pt-4 mt-4">
                <div className="flex items-center gap-2 mb-3">
                  <Activity size={14} className="text-text-muted" />
                  <h3 className="text-sm font-medium text-text">MiniMax MCP</h3>
                  {mcpLoading ? (
                    <Loader2 size={12} className="animate-spin text-text-muted" />
                  ) : mcpStatus ? (
                    <span
                      className={`w-2 h-2 rounded-full ${
                        mcpStatus.connected ? 'bg-status-success-text' : 'bg-status-error-text'
                      }`}
                    />
                  ) : null}
                </div>
                ...
                {mcpStatus && (
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <input
                        ...
                        onChange={async (e) => {
                          setMcpSaving(true);
                          try {
                            await updateMcpConfig({ enabled: e.target.checked });
                            setMcpStatus({ ...mcpStatus, enabled: e.target.checked });
                          } catch (err) {
                    ...
                    {/* API Host input */}
                    <div>
                      <span className="text-xs text-text-muted block mb-1">API Host</span>
                      <div className="flex gap-2">
                        <input
                          type="text"
                          value={mcpStatus.api_host ?? ''}
                          onChange={async (e) => {
                            const newHost = e.target.value;
                            setMcpStatus({ ...mcpStatus, api_host: newHost });
                          }}
                          onBlur={async () => {
                            setMcpSaving(true);
                            try {
                              await updateMcpConfig({ api_host: mcpStatus.api_host ?? '' });
                            } catch (err) {
                              console.error('Failed to update MCP API host:', err);
                            } finally {
                              setMcpSaving(false);
                            }
                          }}
                          ...
                          className="flex-1 rounded-lg border border-border px-3 py-1.5 text-xs font-mono bg-input-bg text-text"
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      <button
                        ...
                        onClick={async () => {
                          if (!showActivity) {
                            try {
                              const activity = await getMcpActivity();
                              setMcpActivity(activity);
                            } catch (err) {
                              console.error('Failed to fetch MCP activity:', err);
                            }
                          }
                      ...
                      >
                        ...
                        Activity Log ({mcpActivity?.calls?.length ?? 0} calls)
                      </button>

                      {showActivity && mcpActivity && (
                        <div className="mt-2 max-h-32 overflow-y-auto bg-border-subtle rounded p-2 text-xs font-mono">
                          {mcpActivity.calls.length === 0 ? (
                            <div className="text-text-muted">No recent calls</div>
                          ) : (
                            mcpActivity.calls.slice().reverse().map((call, i) => (
                              <div key={i} className="py-1 border-b border-border last:border-0">
                                <div className="text-text-muted">
                                  {new Date(call.timestamp).toLocaleTimeString()}
                                </div>
                                <div className="text-text">{call.tool}</div>
                              </div>
                            ))
                          )}
                        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-panel rounded-xl border border-border p-4 space-y-4">
              ...
              <label className="block">
                <span className="text-xs text-text-muted">Memory Hot Reload</span>
                <div className="flex items-center gap-3 mt-1">
                  ...
                  <input
                    ...
                    className="w-20 rounded-lg border border-border px-2 py-1 text-sm bg-input-bg text-text disabled:opacity-50"
                  />
