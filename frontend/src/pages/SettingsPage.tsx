...
import { Save, Loader2, Check, RefreshCw, Database, Globe } from 'lucide-react';
import { catalogStream } from '../api/sse';
import { listModels, type ModelInfo } from '../api/client';
import type { CatalogSSEEvent } from '../api/types';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-panel rounded-xl border border-border p-4 space-y-4">
            ...
            <label className="block">
              ...
              <select
                value={(config.llm_provider as string) ?? 'gemini'}
            ...
            {(config.llm_provider === 'openai') && (
              <label className="block">
                <span className="text-xs text-text-muted">API Base URL</span>
                <input
                  type="text"
                  value={getProviderField('base_url')}
                  onChange={(e) => setProviderField('base_url', e.target.value || null)}
                ...
                <label key={key} className="block">
                  ...
                  {modelsLoading ? (
                    ...
                    <input
                      ...
                      value={current}
                      onChange={(e) => setProviderField(key, e.target.value)}
                      placeholder={key === 'planner_model' ? 'defaults to main model' : ''}
                      className="mt-1 block w-full rounded-lg border border-border px-3 py-2 text-sm font-mono bg-input-bg text-text"
                    />
                  )}
                </label>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-panel rounded-xl border border-border p-4 space-y-4">
              ...
              {([
                ['memory_token_budget', 'Memory Token Budget', 10000],
