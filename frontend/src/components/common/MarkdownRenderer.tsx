...
const components = {
  ...
  code: ({ className, children }: { className?: string; children?: React.ReactNode }) => {
    ...
    if (isBlock) {
      return (
        <pre className="bg-code-bg text-badge-gray-text rounded-md p-3 my-2 overflow-x-auto text-sm">
          <code>{children}</code>
        </pre>
      );
    }
    return (
      <code className="bg-surface-elevated text-text rounded px-1.5 py-0.5 text-sm font-mono break-all">
        {children}
      </code>
    );
  },
