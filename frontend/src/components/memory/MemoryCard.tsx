...
import { Activity, Clock, History, MessageSquare, MessageCircle, Lightbulb, User, Layers, ChevronDown } from 'lucide-react';
...
export function MemoryCard({ entry, sessionId, reviews = [] }: Props) {
  const colors = typeColor[entry.type] ?? defaultColor;
  const [historyOpen, setHistoryOpen] = useState(false);
  const [reviewOpen, setReviewOpen] = useState(false);
  const hasHistory = entry.supersedes !== '';
  const hasReviews = reviews.length > 0 || (entry.review_summary?.total_count ?? 0) > 0;
  // Historical totals from the API (covers all versions + archived reviews)
  const historicalCount = entry.review_summary?.total_count ?? reviews.length;
  const historicalAvg = entry.review_summary?.avg_stars ?? avgReviewStars(reviews);
    ...
    >
      ...
      <div className="flex items-center justify-between mb-2 gap-2">
        ...
        {hasReviews && (
          ...
          >
            <StarRating stars={Math.round(historicalAvg)} />
      ...
      <div className="flex items-center flex-wrap gap-x-3 gap-y-1 text-xs text-text-muted">
        ...
        {entry.last_accessed && (
          <span className="flex items-center gap-1">
            <Clock size={11} />
            {relativeTime(entry.last_accessed)}
          </span>
        )}
        {entry.tags.length > 0 && (
