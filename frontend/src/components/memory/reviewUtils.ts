 ...
 */
export function parseReviewStars(review: MemoryEntry | null | undefined): number {
  if (!review) return -1;
 ...
 */
export function avgReviewStars(reviews: MemoryEntry[]): number {
 ...
 */
export function parseReviewComment(review: MemoryEntry): string {
  return review.content.replace(/^\d★\s*/, '');
}

/**
 * Parse the agent name from a review memory's tags.
 * Tags include the agent name (e.g., "VisualizationAgent").
 */
export function parseReviewAgent(review: MemoryEntry): string {
