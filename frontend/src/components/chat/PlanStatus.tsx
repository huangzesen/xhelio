...
import { ClipboardList, RotateCcw, X, ChevronRight, ChevronDown } from 'lucide-react';
  ...
  return (
    <div className="bg-status-info-bg border border-status-info-border rounded-lg px-3 py-2 text-xs space-y-1">
      ...
      <div className="flex gap-2 mt-1">
        <button
          onClick={() => api.retryPlan(sessionId).catch(() => {})}
          className="flex items-center gap-1 px-2 py-1 rounded text-status-info-text hover:bg-status-info-border/30 transition-colors"
        >
          <RotateCcw size={12} />
          Retry
        </button>
        <button
          onClick={() => {
            api.cancelPlan(sessionId).catch(() => {});
            setPlanStatus(null);
          }}
          className="flex items-center gap-1 px-2 py-1 rounded text-status-info-text hover:bg-status-info-border/30 transition-colors"
        >
