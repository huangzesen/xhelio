...
export function ChatInput({ onSend, onCancel, isStreaming, isCancelling, disabled }: Props) {
  ...
  // Trigger initial completion fetch when text changes from empty
  useEffect(() => {
    if (!text.trim()) {
      setGhostText('');
      cachedCompletions.current = [];
  ...
  }, [text, isStreaming, disabled, onSend, commandMatches, selectedCommandIndex, acceptCommand]);
      ...
      <div className="flex items-end gap-2 max-w-3xl mx-auto">
        <div className="flex-1 relative">
          ...
          <textarea
            ...
            placeholder="Ask about spacecraft data, solar events, or missions..."
            rows={1}
