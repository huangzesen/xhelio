...
interface Props {
  ...
  /** System-generated status text (when LLM produced no text). */
  statusText?: string;
}
...
function AgentToolDetail({ toolCall }: { toolCall: AgentToolCall }) {
  ...
  return (
    <div className="ml-4 py-1 border-l border-border pl-2">
      ...
      {expanded && result && (
        <div className="ml-5 mt-1 text-[10px] text-text-muted font-mono bg-code-bg text-code-text p-2 rounded max-h-96 overflow-auto">
          {result.result != null && result.result !== '' ? truncateResult(result.result, 50000) : (result.text != null && result.text !== '' ? truncateResult(result.text, 50000) : (result.tool_name ? '' : JSON.stringify(result)))}
          {((result.result == null || result.result === '') && ((result.text == null || result.text === '')) && result.tool_name && result.status !== 'error') && (
...
export function ToolCallGroup({
  ...
  return (
    <div className="mx-10 my-1">
      ...
      {/* System-generated status (when LLM produced no text) */}
      {!isStreaming && statusText && (
        <div className="ml-8 mt-1 text-xs text-text-muted italic opacity-70">
          {statusText}
        </div>
      )}
          ...
          {thinkingEvents.length > 0 && (
            <div className="mt-2 pt-2 border-t border-border/50">
              ...
              >
                ...
                <span>Thinking</span>
                <span className="opacity-60">({thinkingEvents.length})</span>
              </button>
              {thinkingExpanded && (
                <div className="ml-4 mt-1 space-y-1">
