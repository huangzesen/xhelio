...
function handleSSEEvent(
  ...
  switch (event.type) {
    case 'text_delta': {
      ...
      // System-generated status — store for activity block, not chat
      if (ev.generated) {
        _pendingStatusText = (_pendingStatusText || '') + ev.text;
        break;
      }
      ...
      set((s) => ({
        toolEvents: [
          ...s.toolEvents,
          {
            id: uid(),
            type: 'call',
            tool_call_id: (event as { tool_call_id?: string }).tool_call_id,
    ...
    case 'round_start': {
      ...
      set((s) => ({
        ...
        roundMarkers: [...s.roundMarkers, { type: 'start', timestamp: ts }],
      }));
      break;
    }
  ...
  sendMessage: async (sessionId: string, message: string, files?: File[]) => {
    ...
    if (message.startsWith('/')) {
      ...
      try {
        ...
        if (command === 'fork' && response.data?.session_id) {
          useSessionStore.getState().setActiveSessionId(response.data.session_id as string);
          useSessionStore.getState().loadSavedSessions();
        }
    ...
    for (const ev of events) {
      ...
      if (ev.type === 'user_message') {
        ...
        if (ev.data?.generated) {
          // System-generated status — attach to most recent round_end marker
          const lastEnd = roundMarkers.filter(m => m.type === 'end').pop();
          if (lastEnd) {
            lastEnd.statusText = (ev.data?.text as string) || ev.msg;
          }
        } else {
          messages.push({
            id: uid(),
      ...
      if (ev.type === 'tool_call' || ev.type === 'tool_started') {
        ...
        toolEvents.push({
          ...
          timestamp: ts,
          agent: ev.agent || undefined,
          result: ev.data?.text as string | undefined,
        });
      }
