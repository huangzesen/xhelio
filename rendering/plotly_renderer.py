...
from agent.fallback_registry import get_fallback
...
def _get_default_width() -> int:
    """Get default width from registry."""
    fb = get_fallback("rendering.default_width")
    if fb:
        return fb["default_value"]
    return 1100


def _get_max_plot_points() -> int:
    """Read max_plot_points from config or registry."""
    fb = get_fallback("rendering.max_plot_points")
    default = fb["default_value"] if fb else 10_000
    try:
...
def _get_webgl_threshold() -> int:
    """Get WebGL threshold from registry."""
    fb = get_fallback("rendering.webgl_threshold")
    if fb:
        return fb["default_value"]
    return 100_000
...
def _extract_index_data(entry: DataEntry) -> tuple[list, bool]:
    ...
    if entry.is_xarray:
        ...
        return [
            pd.Timestamp(t).isoformat() for t in entry.data.coords["time"].values
        ], True
...
def fill_figure_data(
    ...
    for trace_dict in raw_traces:
        ...
        if is_heatmap:
            ...
            if entry.values.ndim == 2 and entry.values.shape[1] > 1:
                ...
                col_names = (
                    list(entry.data.columns)
                    if hasattr(entry.data, "columns")
                    else [str(i) for i in range(ncols)]
                )
            ...
            if is_dt:
                ...
                if x_data:
                    _update_axis_range(
                        datetime_xaxis_ranges, ax_key, x_data[0], x_data[-1]
                    )
            ...
            if n_original > max_pts:
                ...
                if n_original > _get_webgl_threshold() and trace_type == "scatter":
                    trace["type"] = "scattergl"
                    logger.debug(
                        "[Render] Switched '%s' to scattergl (WebGL)", display_name
                    )
    ...
    default_width = _get_default_width()
    width = default_width if n_columns == 1 else int(default_width * n_columns * 0.55)
...
class PlotlyRenderer:
    ...
    def _log(self, msg: str):
        if self.verbose:
            logger.debug("[PlotlyRenderer] %s", msg)
    ...
    def export(self, filepath: str, format: str = "png") -> dict:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"{format.upper()} export failed: {e}",
            }
        ...
        return {
            "status": "error",
            "message": f"{format.upper()} file not created or is empty: {filepath}",
        }
    ...
    def render_plotly_json(
        ...
        for i, trace in enumerate(self._figure.data):
            name = (
                self._trace_labels[i]
                if i < len(self._trace_labels)
                else (trace.name or f"trace_{i}")
            )
            y = trace.y
            if y is not None:
                points = len(y)
                ...
                points = (
                    f"{z_arr.shape[0]}x{z_arr.shape[1]}"
                    if z_arr.ndim == 2
                    else len(trace.z)
                )
            else:
...
# ---------------------------------------------------------------------------
# Module-level singleton (mirrors data_ops/store.py pattern)
# ---------------------------------------------------------------------------

_global_renderer: Optional[PlotlyRenderer] = None


def get_renderer() -> PlotlyRenderer:
    """Return the global PlotlyRenderer instance."""
    if _global_renderer is None:
        raise RuntimeError("No PlotlyRenderer set. Call set_renderer() first.")
    return _global_renderer


def set_renderer(renderer: PlotlyRenderer) -> None:
    """Set the global PlotlyRenderer instance."""
