...
def replay_session(
    ...
    *,
    ...
    if use_cache:
        ...
        if data_dir.exists() and (data_dir / "_index.json").exists():
            cache_store = DataStore()
            n = cache_store.load_from_directory(data_dir)
            logger.info("Replay: loaded %d cached entries from %s", n, data_dir)
        else:
            logger.warning(
...
def _list_sessions() -> None:
    ...
    for d in sorted(sessions_dir.iterdir()):
        ...
        has_data = (d / "data" / "_index.json").exists()
