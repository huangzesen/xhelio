...
TOOL_STYLE = {
    ...
    "generate_jsx_component": {"color": "#B79F00", "symbol": "square",  "label": "jsx"},
}
...
TOOL_STYLE_ORPHAN = {
    ...
    "generate_jsx_component": {"color": "#E6D86A", "symbol": "square",  "label": "jsx"},
}
...
def build_figure(pipeline: list[dict], session_id: str) -> go.Figure:
    ...
    for rec in pipeline:
        ...
        if family_members and state_count > 1:
            ...
            # For fetch_data, use dataset_id.parameter_id from args for display
            if rec["tool"] == "fetch_data":
                dataset_id = rec["args"].get("dataset_id", "")
                parameter_id = rec["args"].get("parameter_id", "")
                if dataset_id:
                    display_name = dataset_id
                    if parameter_id:
                        display_name += f".{parameter_id}"
                    wrapped = _wrap_text(display_name)
                    node_text = style["label"] + f"<br><sub>{wrapped}</sub>"
                else:
                    # Fallback to outputs if no dataset_id in args
