...
def test_2_fetch_describe(results: TestResults):
    ...
    results.check(
        ...
        "delegate_to_envoy" in tools or "fetch_data" in tools,
        f"tools: {tools}",
    )
    results.check(
    ...
    results.check(
        ...
        has_tool(r, "describe_data") or has_tool(r, "delegate_to_envoy"),
        f"tools: {tool_names(r)}",
    )
    results.check(
    ...
    results.check(
        ...
        has_tool(r, "list_fetched_data") or has_tool(r, "delegate_to_envoy"),
        f"tools: {tool_names(r)}",
    )
    results.check(
...
def test_3_compute_save(results: TestResults):
    ...
    results.check(
        ...
        has_tool(r, "save_data") or has_tool(r, "delegate_to_envoy"),
        f"tools: {tool_names(r)}",
    )
...
def test_4_psp_electron_pad_spectrogram(results: TestResults):
    ...
    results.check(
        ...
        "style_plot" in tools or "delegate_to_viz" in tools,
        f"tools: {tools}",
    )
...
def test_5_cross_instrument_spectrogram(results: TestResults):
    ...
    results.check(
        ...
        any(t in tools for t in ["delegate_to_envoy", "fetch_data",
                                  "delegate_to_viz", "plot_data"]),
        f"tools: {tools}",
    )
