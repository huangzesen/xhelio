...
async def execute_command(session_id: str, req: CommandRequest):
    ...
    if cmd == "fork":
        # Fork session with independent history and fresh agents
        loop = asyncio.get_running_loop()

        def _fork():
            new_state, _meta, _display_log, _event_log = session_manager.fork_session(
                session_id
            )
            return new_state.session_id

        new_id = await loop.run_in_executor(_thread_pool, _fork)
        return CommandResponse(
            command=cmd,
            content=f"Forked session. New session: `{new_id}`\n\nThe new session has independent conversation history and fresh agents with current memory.",
...
async def list_eureka_suggestions(
    ...
    """List eureka suggestions with optional filters."""
    from agent.eureka_store import EurekaStore
    from dataclasses import asdict

    store = EurekaStore()
    entries = store.list_suggestions(session_id=session_id, status=status)
    return {"suggestions": [asdict(s) for s in entries]}


@router.patch("/eureka/suggestions/{suggestion_id}")
async def update_eureka_suggestion(suggestion_id: str, body: dict):
    """Update eureka suggestion status."""
    from agent.eureka_store import EurekaStore

    status = body.get("status")
    if status not in ("proposed", "approved", "executed", "rejected"):
        raise HTTPException(status_code=400, detail="Invalid status")
    store = EurekaStore()
...
@router.post("/eureka/chat")
async def eureka_chat(session_id: str, req: dict):
    """Send a chat message to the EurekaAgent and stream the response.

    Returns SSE stream with eureka_chat_response events.
    Returns 409 if EurekaAgent is busy (processing auto-extraction or another chat).
    """
    from sse_starlette.sse import EventSourceResponse
    ...
    if eureka_agent.state == AgentState.ACTIVE:
        raise HTTPException(
            status_code=409,
            detail="EurekaAgent is busy. Please wait for current analysis to complete.",
        )
    ...
    # Create an SSE bridge for streaming responses
    queue: asyncio.Queue = asyncio.Queue()

    from agent.eureka_store import EurekaStore, EurekaChatMessage
    from dataclasses import asdict
    from datetime import datetime, timezone
    ...
    async def event_generator():
        try:
            ...
            await queue.put(
                {
                    "type": "eureka_chat_message",
                    "role": "user",
                    "content": message,
                }
            )

            # Send message to EurekaAgent and get result (run in thread pool to avoid blocking event loop)
            loop = asyncio.get_running_loop()
            result = await loop.run_in_executor(
                _thread_pool,
                lambda: eureka_agent.send(
            ...
            if result.get("failed"):
                response_content = f"Error: {result.get('text', 'Unknown error')}"
                await queue.put(
                    {
                        "type": "eureka_chat_response",
                        "role": "assistant",
...
@router.get("/eureka/chat/history")
async def get_eureka_chat_history(limit: int = 50):
    """Get eureka chat history."""
    from agent.eureka_store import EurekaStore, EurekaChatMessage
    from dataclasses import asdict

    store = EurekaStore()
    messages = store.get_chat_history(limit=limit)
...
async def update_mcp_config(req: dict):
    ...
    if "api_host" in req:
        MiniMaxMCPClient.set_api_host(req["api_host"])
