...
class OperationsLog:
    ...
    def record(
        ...
        with self._lock:
            ...
            input_producers = {
                label: self._current_producers[label]
                for label in input_list
                if label in self._current_producers
            }
            record = {
    ...
    def get_pipeline(self, final_labels: set[str]) -> list[dict]:
        ...
        def _resolve_producer(rec: dict, label: str) -> dict | None:
            ...
            if label in ip:
                producer_id = ip[label]
                prod = rec_by_id.get(producer_id)
                if prod is not None:
                    return prod
            # Fallback: global last_producer
            return last_producer.get(label)
        ...
        for rec in reversed(records):
            if rec["tool"] == "render_plotly_json" and rec["status"] == "success":
                ...
                while product_queue:
                    ...
                    prod = _resolve_producer(context_rec, label)
                    if prod is None:
                        continue
    ...
    def get_state_pipeline(self, render_op_id: str, final_labels: set[str]) -> list[dict]:
        ...
        # Build helpers (same as get_pipeline)
        last_producer: dict[str, dict] = {}
        for rec in records:
            if rec["status"] != "success":
                continue
    ...
    def get_pipeline_mermaid(self, final_labels: set[str]) -> str:
        ...
        for rec in pipeline:
            ...
            # fall back to label_producer for old records.
            seen_edges: set[tuple[str, str]] = set()
            ip = rec.get("input_producers", {})
            for inp in rec["inputs"]:
                src = ip.get(inp) or label_producer.get(inp)
                if src:
