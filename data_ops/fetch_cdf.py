...
def inspect_dataset(
    ...
    """Download one CDF file for a dataset and return its actual variables.

    Use this to see ground-truth variable names before calling fetch_data,
    especially when list_parameters shows parameters marked NOT_IN_DATA_FILES.

    Steps:
    1. Discover CDF files for the time range via CDAWeb REST API.
    2. Download only the first file (cached automatically).
    3. Inspect the file's data variables with ``inspect_cdf_variables()``.
    4. Run ``_sync_metadata_with_data_cdf()`` to update override annotations.

    Args:
        dataset_id: CDAWeb dataset ID (e.g., "PSP_SWP_SPC_L3I").
        time_min: ISO start time.
        time_max: ISO end time.

    Returns:
        Dict with keys: dataset_id, file_inspected, variables.
    """
    file_list = _get_cdf_file_list(dataset_id, time_min, time_max)
    first_file = file_list[0]
    local_path = _download_cdf_file(first_file["url"], CACHE_DIR)

    variables = inspect_cdf_variables(local_path)

    # Side-effect: sync metadata annotations so list_parameters picks up changes
    _sync_metadata_with_data_cdf(
        dataset_id, local_path, source_url=first_file["url"],
    )

    get_event_bus().emit(CDF_FILE_QUERY, agent="cdf", level="info", msg=f"[CDF] Inspected {dataset_id}: {len(variables)} data variables in {local_path.name}")

    return {
        "dataset_id": dataset_id,
        "file_inspected": local_path.name,
        "variables": variables,
    }
